...
function registerHooks () {
    ...
    Hooks.on('chatMessage', (chatLog, message, options) => {
        if (message.match(`^/(attack|a|check|c|concentration|con|damage|d|heal|h|healing|item|save|s|skill|k|tool|t) `) || message.match(`^/(concentration|con)`)) {
...
function parseChatCommand(message) {
    const stringNames = ['attack', 'award', 'a', 'check', 'concentration', 'con', 'c', 'damage', 
        'd', 'heal', 'healing', 'h', 'item', 'save', 'skill', 's', 'k', 'tool', 't']
    const shorthands = { a: 'attack', c: 'check', con: 'concentration', d: 'damage', h: 'heal', k: 'skill', s: 'save', t: 'tool' }
    const rgx = new RegExp(`/(?<type>${stringNames.join('|')}\b)\s*(?<config>.*)`, 'gi')
    const match = rgx.exec(message)
    if (!match) return
